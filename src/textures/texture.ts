...
export class Texture2D<T extends PixelArray | HTMLElement> extends Texture {
  ...
  public static async loadEnv(
    ...
    const image = await fetchEXR(url);
    const tex = new Texture2D<PixelArray>(
      image.data,
      image.width,
      image.height,
      image.formatGL,
...
function fetchEXR(url: string): Promise<Image2D<PixelArray>> {
  return new Promise((res, rej) => {
    var a = new EXRLoader().setDataType(THREE.HalfFloatType);
    var b = a.load(url, () => {
      res({
        data: b.image.data,
        width: b.image.width,
        ...
        formatGL: WebGL2RenderingContext.RGBA,
        internalFormatGL: WebGL2RenderingContext.RGBA16F,
        typeGL: WebGL2RenderingContext.HALF_FLOAT
      });
    });
  });
}
